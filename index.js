...
const exportPath = process.env.EXPORT_PATH || '/usr/src/app/music_list';
...
const overwriteRadioFile = (data) => {
  LoggerLog(
    `ready to write file 【${path.join(exportPath, xiaomusic_music_list)}】...`,
  );
  try {
    fs.writeFileSync(
      path.join(exportPath, xiaomusic_music_list),
      JSON.stringify(data),
      "utf-8",
    );
    LoggerLog(
      `write file 【${path.join(exportPath, xiaomusic_music_list)}】 sucsess`,
    );
  } catch (err) {
    LoggerError(
      `write file 【${path.join(exportPath, xiaomusic_music_list)}】 failed: ${err}`,
    );
  }
};
...
const ffmpegInstance = (
  format,
  { inputAudio, inputImage, outputAudio, title, artist, album },
  { start, end, error, stderr },
) => {
  ...
  const commonOptions = [
    ...
    "1", // 选择封面图片流
    "-disposition:v",
    "attached_pic", // 将封面图片标记为附加图片
    "-c:v",
    imageCodec, // 根据图片格式选择适当的编码器（JPEG 或 PNG）
    "-vf",
    "scale=iw/2:ih/2", // 将图片尺寸缩小为原始尺寸的 50%
  ];
  ...
  if (format === "mp3") ffmpegCommand.addOption("-id3v2_version", "3");

  if (start) ffmpegCommand.on("start", start);

  if (stderr) ffmpegCommand.on("stderr", stderr);

  if (end) ffmpegCommand.on("end", end);

  if (error) ffmpegCommand.on("error", error);
 ...
 */
const download = async ({ url, format, name, id, detail }) => {
    ...
    try {
      ...
      axios({
            ...
            ffmpegInstance(
              ...
              },
              {
                ...
                // },
                  ...
                  LoggerLog(
                    `[${name}](${id}) Metadata and cover modifications are complete.`,
                  );
                  ...
                  fs.rename(tempFile, inputFile, async (err) => {
                    if (err) {
                      LoggerError(
                        `[${name}](${id}) Failed to overwrite the file: ${err}`,
                      );
                    }
                    return resolve();
                  });
        ...
        .catch((err) => {
          // 图片下载，什么都不做
          LoggerError(
            `[${name}](${id}) Unable to download cover image: ${err}`,
          );
          return resolve();
        });
    } catch (e) {
      ...
      Promise.all([
        ...
        if (method.includes("local")) {
          ...
          for (let i = 0; i < musics.length; i++) {
            ...
            const downloaded = old_local_data.find(
              (it) => path.parse(it).name === `${name}.${id}`,
            );
            ...
            console.log("\n");
            LoggerLog(`[${i}][${name}](${id}) is not in local, ready to download...`);
            ...
            try {
              ...
              const {
                data: { url, type },
              } = songDownloadResponse.body;

              LoggerSuccess(
                `[${name}](${id}) get download url success: `,
                url,
                type,
              );
              ...
              try {
                ...
                LoggerError(
                  `[${name}](${id}) download failed: ${e.toString()}\n CONTINUE TO DOWNLOAD NEXT SONG...`,
                );
                continue;
              }
            } catch (e) {
